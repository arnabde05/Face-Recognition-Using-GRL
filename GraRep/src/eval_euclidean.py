...
for i, test_emb in enumerate(test_embeddings):
    ...
    print(f"Best match is with train image {match_index+1} (subject {match_id}), with Euclidean distance of {min_distance:.4f}")
    
    if min_distance <= threshold and test_ids[i] == match_id:
